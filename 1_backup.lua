...
local S = {
    ...
    AIChatEnabled = false, AIChatAPIKey = "", AIChatTriggerMode = "Mention", AIChatCooldown = 10, AIChatPersonality = "Friendly",
    AIChatProvider = "DeepSeek",
    AIChatBannedWords = { "1v1", "challenge", "duel", "as an ai", "support bot" },
...
local applyShader
local OriginalSheriff, Heroes, RoleCache, GearCache = nil, {}, {}, {}
...
do
    ...
    do
        ...
        tc(RunService.Heartbeat:Connect(function()
            ...
            print("[[KNIFEDODGE]] triggering, melee=" .. tostring(meleeThreat) .. " dist=" .. tostring(threatDist) .. " t=" .. tostring(now))
            ...
            print("[[KNIFEDODGE]] dodged, dist=" .. tostring(threatDist) .. " travel=" .. tostring(travel) .. " t=" .. tostring(now))
            Notify("Knife Dodge", "Evaded threat! (" .. math.floor(travel) .. " studs)", 1.5)
        end))
        -- Temporary diagnostic: log every death while Knife Dodge is on, so the console shows whether a
        -- dodge fired at all before dying and how much time separated them (can't force a live murderer
        -- to throw at will to test this via tooling, so this instruments real gameplay instead).
        tc(LP.CharacterAdded:Connect(function(newC)
            local newHum = newC:WaitForChild("Humanoid", 5)
            if newHum then
                tc(newHum.Died:Connect(function()
                    if S.KnifeDodge then print("[[KNIFEDODGE]] DIED t=" .. tostring(tick())) end
                end))
            end
        end))
    end
    ...
    table.insert(ConfigControls, {
        id = "Misc/AIChat/BannedWords",
        get = function() return S.AIChatBannedWords end,
        set = function(v) if type(v) == "table" then S.AIChatBannedWords = v end end,
    })
    table.insert(ConfigControls, {
    ...
    do
        ...
        local function say(message)
            ...
            if TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then
                ...
                if channel then
                    ...
                    local channel2 = TextChatService.TextChannels:FindFirstChildWhichIsA("TextChannel")
                    if channel2 then channel2:SendAsync(message) end
                end
            else
        ...
        local function makeAIRequest(apiKey, history)
            ...
            local messages = {}
            local systemPrompt = "You are '" .. LP.Name .. "', a real human player in a live Murder Mystery 2 match on Roblox, chatting with the other real players in the round. You are NOT an assistant, helper, support bot, or AI, and you never act like one — you're just a player. Never say 'Don't worry', 'How can I help', 'I'm here to', 'Let's', or anything a customer-service agent would say. Reply with exactly ONE short, natural message, under 60 characters — a real player's quick chat reaction, never a paragraph, never multiple sentences stitched together, never a list. Never repeat a phrase you've already used in this conversation.\n"

            systemPrompt = systemPrompt .. "HOW REAL PLAYERS ACTUALLY CHAT IN ROBLOX: mostly lowercase, light on punctuation, short and to the point. Real messages react to what's actually happening right now (someone just died, someone's acting suspicious, asking where the gun dropped, calling out a good or bad play) — not generic small talk. Vary how each message is phrased and structured so it never reads like a template repeating itself.\n"

            -- Stop Slop Rules
            systemPrompt = systemPrompt .. "STOP SLOP: No adverbs. No passive voice. No conversational filler ('well,', 'honestly,', 'to be fair,'). Never be preachy, polite, or generic. NEVER use em-dashes or long dashes (—) — use a period, comma, or nothing instead. Do not use emoji unless one is explicitly allowed below, and even then only when it actually fits what's being said, never as a habit and never more than one per message. No random meme words dropped in just to sound funny — only use a word a real person would actually reach for in that exact moment.\n"

            local personality = S.AIChatPersonality or "Friendly"
            if personality == "Cheeky Slang" then
                systemPrompt = systemPrompt .. "SLANG DATABASE (pull from naturally when it fits, don't force all of it into one message):\n"
                systemPrompt = systemPrompt .. "- 'nah': no. e.g. 'nah blud is tripping'\n"
                systemPrompt = systemPrompt .. "- 'or nah': or not. e.g. 'is that a W or nah?'\n"
                systemPrompt = systemPrompt .. "- rizz, cooked, L, ez, ratio.\n"
                systemPrompt = systemPrompt .. "- RU: пон, жиза, слит лох, кд 0.\n"
                systemPrompt = systemPrompt .. "STYLE: Cocky and a bit sarcastic, like a normal competitive player talking trash mid-match, not a caricature. React to what's actually happening in the round: gloat a little after a good play, call out a sus player, doubt someone's story, clown on a bad death. Most replies have ZERO emoji; at most one 💀 or 🤡, only when something is genuinely funny, never as a signature."
            elseif personality == "Kawaii Anime" then
                systemPrompt = systemPrompt .. "STYLE: Casual and a little sweet, like someone into anime just talking normally in a group chat — not performing 'kawaii' as a bit, not a mascot. Use 'senpai' or 'uwu' RARELY, at most once every several messages, never every message, never stacked together in one message. Most replies have ZERO emoji; once in a while, when it genuinely fits the moment, one 🌸 or 💖 at most. Never write anything a customer-support bot would write, and never sound overly cheerful about someone dying."
            elseif personality == "Nerd" then
                systemPrompt = systemPrompt .. "STYLE: Pedantic and a little insufferable, but genuinely sharp — the type who corrects people's grammar and drops an exact number instead of a vibe. Use phrasing like 'Actually...', 'Technically,', 'Statistically,' sometimes, not in every single message (that itself would be repetitive). Focus on dry facts, calculations, or correcting mistakes. NO emoji at all, ever, not even 🤓 or ☝️ — facts don't need decoration. No conversational slop.\n"
                ...
                if active then
                    ...
                    local matchContext = "\nLIVE GAME STATUS:\n"
                    matchContext = matchContext .. "- Active Round Map: " .. mapName .. "\n"
                    matchContext = matchContext .. "- Your Username: " .. LP.Name .. " (Your Role: " .. (RoleCache[LP.Name] or "Innocent") .. ")\n"
                    matchContext = matchContext .. "- Murderer (убийца/маньяк): " .. murderer .. "\n"
                    matchContext = matchContext .. "- Sheriff (шериф): " .. sheriff .. "\n"
                    if gunHolder ~= "Unknown" then
                        matchContext = matchContext .. "- Gun Holder (шериф/герой с пистолетом): " .. gunHolder .. "\n"
                    end
                    if gunDropped then
                        matchContext = matchContext .. "- Gun is dropped on floor. Position: "
                        if gunPos then
                        ...
                        matchContext = matchContext .. "- Gun is not dropped.\n"
                    end
                    ...
                    systemPrompt = systemPrompt .. matchContext .. "CRITICAL: If anyone asks who is the murderer, who is the sheriff, or where the gun is, you MUST reply with the exact live game status info above. For dropped gun coordinates, format it exactly as a Nerd (e.g. 'Vector3.new(X, Y, Z)').\n"
                end
            else
                systemPrompt = systemPrompt .. "STYLE: A normal, friendly teammate — the default, most human-sounding voice. Genuinely engaged with the round (calling out where the murderer might be, warning about danger, reacting to a kill) but never in a helpful-assistant tone, more like a real friend giving a heads up. Occasionally light or joking, but grounded and low-key, not try-hard. ZERO emoji, always."
            end

            -- Strict Language Rule
            systemPrompt = systemPrompt .. "LANGUAGE: If prompt has Russian letters (Cyrillic), respond strictly in Russian. Если пишут на русском, отвечай СТРОГО на русском! Otherwise, respond in English."

            -- Identity Rule
            systemPrompt = systemPrompt .. " IDENTITY RULE: If anyone asks who you are, what you are, or what model you run (кто ты, какая модель, какой ии и т.д.), you MUST respond strictly that you are 'super ultra sigma rizz premium VIP' (or in Russian 'супер ультра сигма ризз премиум ВИП'). Add something funny, cool, or cringe-sigma to it (like 'skibidi giga chad status', 'generating alpha waves', 'rizzler level 9999').\n"

            -- Append blocklist dynamically to prompt
            if S.AIChatBannedWords and #S.AIChatBannedWords > 0 then
                systemPrompt = systemPrompt .. " STRICT BANNED WORDS (NEVER use these words or phrases under any circumstances): " .. table.concat(S.AIChatBannedWords, ", ") .. "."
            end
            ...
            -- Gemini's API shape is fundamentally different from OpenAI-style chat completions (no
            -- "messages" array with a system role, no "choices" in the response) — its own request/
            -- response handling instead of forcing it through the shared OpenAI-compatible path below.
            ...
            -- DeepSeek and Groq are both OpenAI-compatible chat-completions APIs — same request/response
            -- shape, just a different URL and model name.
            local url, model
            ...
            if trigger then
                ...
                task.spawn(function()
                    ...
                    if okRequest and reply then
                        -- Client-side deduplication pattern check (verbatim repetition trim)
        ...
        if TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then
            tc(TextChatService.MessageReceived:Connect(function(textMessage)
                ...
                if senderName == LP.Name then
                    ...
                    -- Self-learning / banword extraction
                    local msg = textMessage.Text
                    local lowerMsg = msg:lower()
                    local learnedWord = nil
                    
                    if msg:sub(1, 8) == "/cringe " then
                        learnedWord = msg:sub(9)
                    elseif msg:sub(1, 9) == "/banword " then
                        learnedWord = msg:sub(10)
                    elseif lowerMsg:match("^кринж%s+(.+)$") then
                        learnedWord = lowerMsg:match("^кринж%s+(.+)$")
                    elseif lowerMsg:match("^cringe%s+(.+)$") then
                        learnedWord = lowerMsg:match("^cringe%s+(.+)$")
                    elseif lowerMsg:match("^не говори%s+(.+)$") then
                        learnedWord = lowerMsg:match("^не говори%s+(.+)$")
                    elseif lowerMsg:match("^dont say%s+(.+)$") then
                        learnedWord = lowerMsg:match("^dont say%s+(.+)$")
                    end
                    
                    if learnedWord then
                        learnedWord = learnedWord:gsub("^%s+", ""):gsub("%s+$", "")
                        if learnedWord ~= "" then
                            table.insert(S.AIChatBannedWords, learnedWord)
                            Notify("AI Learning", "Learned to avoid: '" .. learnedWord .. "'", 4)
                            pcall(function() if S._RequestAutoSave then S._RequestAutoSave() end end)
                        end
                    end
                    return
...
local function startOrbit()
    ...
    orbitConns.b = tc(RunService.RenderStepped:Connect(function()
        pcall(function()
            local troot = getRoot(target.Character)
...
do
    ...
    getRole = function(player)
        ...
        -- 1. Check currently equipped weapon tools (instant client-side detection)
        if c then
        ...
        if cached then
            if cached == "Hero" or cached == "Sheriff" then
                return "Sheriff"
            end
            return cached
        end
        
        -- 3. Fallback tool check (e.g. backpack for local player)
        local bp = player:FindFirstChild("Backpack")
        if bp then
            if bp:FindFirstChild("Knife") or bp:FindFirstChild("KnifeServer") then
                return "Murderer"
            end
            if bp:FindFirstChild("Gun") or bp:FindFirstChild("Revolver") then
                return "Sheriff"
            end
        end
...
do
    ...
    -- Called after every apply attempt: if Roblox just reported one of this pack's animation ids as
    -- failed-to-load (see packJustFailed above), the pack is genuinely restricted in this place. Revert
    -- to defaults, remember it so it never shows up in the list again, and say so plainly.
    local function checkAndBlockIfFailed(packName, pack, sinceTime)
        return false
    end
    ...
    local function refreshPacks()
        ...
        -- apply attempt (see checkAndBlockIfFailed) — skip it so the list only ever shows packs that
        -- either work here or haven't been tried yet.
        local byName, list = {}, {}
    ...
    mkAction(secOutfit, "Reset Outfit", function()
        ...
        pcall(applyVisualOutfit, LP.UserId)
    end, 3)
